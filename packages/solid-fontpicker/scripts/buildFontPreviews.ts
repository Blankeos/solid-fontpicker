...
function printHelp() {
  ...
  l.push(
    "\t--googlefonts: Retrieve fonts from Google (as opposed to manual preview generation). Implied by --lite."
  )
...
class GoogleFonts {
  ...
  static async fetchAll(
    ...
    const fontInfos: GoogleFontInfo[] = await GoogleFonts.getFontList(
    ...
    for (const info of fontInfos) {
      ...
      const font: DownloadedFont = {
        ...
        variants: GoogleFonts.shortVariants(info),
  ...
  static async getFontList(
    ...
    const localJsonFile = GoogleFonts.fontPath + "/fonts.json"
    ...
    if (!fs.existsSync(localJsonFile) || (!noReplaceOldFontInfos && age > oneWeekMillis)) {
      ...
      const url = apiBaseUrl + GoogleFonts.apiKey + "&sort=popularity"
    ...
    if (filterList?.length > 0) {
      filterFunc = (font) => filterList.includes(font["family"])
    } else {
      filterFunc = (font) => {
    ...
    fontInfos.sort((fontA: GoogleFontInfo, fontB: GoogleFontInfo) =>
      fontA?.family?.localeCompare(fontB?.family)
    )
    return fontInfos
  }
}
...
class FontPreviewBuilder {
  ...
  static async generatePreview(fonts: DownloadedFont[], outputPath: string, sliceSize: number) {
    ...
    FontPreviewBuilder.outputPath = outputPath
    ...
    for (const font of fonts) {
      ...
      font["top"] = (num % sliceSize) * FontPreviewBuilder.cellHeight
      num++
    }
    await Promise.all([
      FontPreviewBuilder.makeJson(fonts),
      FontPreviewBuilder.makeImages(fonts, sliceSize),
      FontPreviewBuilder.makeCss(fonts, sliceSize),
      FontPreviewBuilder.makeHtml(fonts),
    ])
  }
  ...
  static async makeJson(fonts: DownloadedFont[]) {
    ...
    fs.writeFileSync(
      FontPreviewBuilder.outputPath + "/fontInfo.json",
  ...
  static async makeImages(fonts: DownloadedFont[], sliceSize: number) {
    ...
    for (let i = 0; i < fonts.length / sliceSize; i++) {
      ...
      await FontPreviewBuilder.makeImage(
        slice,
        FontPreviewBuilder.outputPath + "/sprite." + (i + 1)
      )
    }
  }
  ...
  static async makeImage(fonts: DownloadedFont[], outFile: string) {
    ...
    const dstH = Math.ceil(FontPreviewBuilder.cellHeight * fonts?.length * outScale)
    const fontSize = Math.ceil(22 * outScale)
    const indent = Math.ceil(10 * outScale)
    const baseline = Math.ceil(FontPreviewBuilder.cellHeight * 0.7 * outScale)
  ...
  static async makeCss(fonts: DownloadedFont[], sliceSize: number) {
    ...
    fs.writeFileSync(FontPreviewBuilder.outputPath + "/font-previews.css", css.join("\n"))
  }
  ...
  static async makeHtml(fonts: DownloadedFont[]) {
    ...
    for (const font of fonts) {
      html.push(
        '    <div class="font-preview-' + font["sanename"] + '" title="' + font["name"] + '"></div>'
      )
    }
    html.push("</body>")
    html.push("</html>")
    fs.writeFileSync(FontPreviewBuilder.outputPath + "/font-previews.html", html.join("\n"))
  }
}
...
async function main() {
  ...
  if ("num-fonts" in opts) {
    numFonts = Number.parseInt(opts["num-fonts"] as string)
  }
  if ("slice-size" in opts) {
    sliceSize = Number.parseInt(opts["slice-size"] as string)
  }
  if ("filter" in opts) {
  ...
  if (googlefonts) {
    ...
    const apiKey: string = fs.existsSync(apiKeyFile)
      ? fs.readFileSync(apiKeyFile)?.toString()?.trim()
      : ""
    if (apiKey.length < 20) {
      throw new Error(
